...
void playGame() {
	...
	cin >> guessCol;
	cout << "Enter direction (n=north, s = south etc.) (\"n\", \"e\", \"s\" or \"w\"): ";
	cin >> guessDirection;
...
void testJumble() {
	...
	int trueSize = hiddenWord.size() * 3;
	if(trueSize != size){
		cerr << "sizes do not match." <<endl;
		}
	if(row < 0 || row > hiddenWord.size()* 4){
		cerr << "Could not get a row" << endl;
		}
	if(col < 0 || col > hiddenWord.size()* 3){
		cerr << "Could not get a column" << endl;
		}
	char dir = jp->getDirection();
		if (dir != 'n' &&  dir != 'e' && dir != 's' && dir != 'w'){
			cerr << "The inputted direction is invalid" << endl;
		}
	...
	//easy size
	trueSize = hiddenWord.size() * 2;
	jp = new JumblePuzzle(hiddenWord, "easy");

	if(jp->getSize()==trueSize){
	...
	if(jumble[row][col] != hiddenWord.at(0)){
		cerr << "The letter does not match the row and column, therefore Invalid." << endl;
		}
		for (int i = 1; i < hiddenWord.size(); i++){
			...
			if (jumble[row][col] != hiddenWord.at(i)){
				cerr <<"Invalid Puzzle Formed" << endl;
				}
		}

	//------------------------------------------------------------------------------------
	//Unit case 4 Exceptions
	try
		{
			//too short
			jp = new JumblePuzzle("no", "easy");
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
		...
		{
			//too long
			jp = new JumblePuzzle("supercalifragilisticexpialidocious", "easy");
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
		...
		//------------------------------------------------------------------------------------
		//Unit case 5 Difficulty

		try
		{
			//has to be one of the options
			jp = new JumblePuzzle(hiddenWord, "beginner"); 
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
	...
	//same as the original testJumble
...
int main() {
	...
	//comment out whatever you do not want to see
	testJumble();
	playGame();
